...
public class UsageDataJpaServiceControllerServiceAdaptorTest {
	...
	public void setUpController() throws Exception {
		...
		usageAdaptor.initialize(COMMUNICATION_WIRING.getUrlLibrary(), COMMUNICATION_WIRING.getHttpRestClient());
	...
	// when(sqlProxy.fetchUsageActivities(eq(DOCUMENT_ID), eq(EVERYTHING))).thenReturn(new
	// UsageActivities(singletonList(DEFAULT_ACTIVITY)));
	...
	// adaptors.fetchUsageActivities(DOCUMENT_ID, new Bounds(DEFAULT_BOUNDS.offset,
	// DEFAULT_BOUNDS.limit, Long.MIN_VALUE, Long.MAX_VALUE));
	// verify(sqlProxy, times(1)).fetchUsageActivities(eq(DOCUMENT_ID), eq(DEFAULT_BOUNDS));
	// }
