...
public class GraphQlController {
	...
	public GraphQlController(ClusterDataServiceConfiguration configuration) {
	...
	private List<GraphQLFieldDefinition> buildFieldDefinitions(ModelAdaptor adaptor) {
		...
		for (final String dataType : adaptor.getDataTypes()) {
			...
			fields.add(newFieldDefinition().name(dataType).type(dataTypeObjectType).argument(arguments).dataFetcher(fetcher).build());
		}
	...
	private GraphQLObjectType buildObjectType(String dataType, ModelAdaptor adaptor) {
		...
		if (adaptor.isSimpleRepresentable(dataType)) {
			...
			builder.field(newFieldDefinition().name("formattedTimeOfEvent").type(GraphQLString).argument(format(new LinkedList<>()))
					.dataFetcher(new FormattedTimeOfEventDataFetcher(adaptor)).build());
	...
	private GraphQLObjectType createInvertedIndexLookupTypeForSelector(String dataType, ModelAdaptor adaptor) {
		...
		for (final Indexable indexable : adaptor.getIndexablesFor(dataType)) {
			builder.field(newFieldDefinition().name(indexable.path + "_" + indexable.documentType).type(list(new GraphQLTypeReference(indexable.documentType)))
	...
	private GraphQLOutputType buildGeoLocationType(ModelAdaptor adaptor) {
		...
		builder.field(newFieldDefinition().name("longitude").type(GraphQLBigDecimal).dataFetcher(new GeoLocationValueFetcher("longitude", adaptor)).build());
		builder.field(newFieldDefinition().name("latitude").type(GraphQLBigDecimal).dataFetcher(new GeoLocationValueFetcher("latitude", adaptor)).build());
		return builder.build();
	}
